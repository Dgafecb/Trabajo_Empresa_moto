    ...
    private boolean modificarTrabajador(){
        if (this.panelRegistrosTrabajadores.jTable2.getSelectionModel().isSelectionEmpty() == false) {
            if (this.panelRegistrosTrabajadores.jTable2.isEditing()) {
                ...
                Integer privilegio = Integer.valueOf(this.panelRegistrosTrabajadores.jTable2.getValueAt(this.panelRegistrosTrabajadores.jTable2.getSelectedRow(), 5).toString());
                ...
                if (consultas.update(modelo)) {
                    lista_trabajadores.add(modelo);
                    int index = (lista_trabajadores.findTrabajador(lista_trabajadores, DNI));
                    lista_ventas.remove(index);
                    lista_ventas.add(index, modelo);
                    fillTableTrabajadores(lista_trabajadores);
                    return true;
                } else {
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == this.panelRegistrosTrabajadores.btnTrabajadorAgregar) { 
            ...
            Thread hilo = new Thread(){
                @Override
                public void run() {
                   if(modificarTrabajador()) mensaje("OPERACION REALIZADA");
                   else mensaje("OPERACION FALLIDA");
                    return ;
                }
            };
            ...
            Thread hilo = new Thread(){
                @Override
                public void run() {
                   if(eliminarTrabajador()) mensaje("OPERACION REALIZADA");
                   else mensaje("OPERACION FALLIDA");
                    return ;
                }
            };
            ...
            if (me.getClickCount() == 2){
                ...
                if(ATrabajador.size()>0){
                    ...
                    if(segundosTardanza.size()>0){
                        ...
                        String hora = String.valueOf(hh);if(hora.length()<2) hora="0"+hora;
                        String minuto= String.valueOf(mm);if(hora.length()<2) minuto="0"+minuto;
                        String segundo = String.valueOf(ss);if(hora.length()<2) segundo="0"+segundo;
