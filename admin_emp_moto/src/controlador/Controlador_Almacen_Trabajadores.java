...
public class Controlador_Almacen_Trabajadores implements ActionListener {
     private Controlador_trabajador ControladorTrabajador;
    ...
    public Controlador_Almacen_Trabajadores(Controlador_trabajador controladorTrabajador, Ventana_Trabajador ventanaTrabajador) {
     ...
     private void iniciarComponentes() {
        this.ventanaTrabajador = ControladorTrabajador.getVentanaTrabajador();
        this.panelInventario = ControladorTrabajador.panel_almacen;
    ...
    private void buscarAlmacen(String referencia, int numero) {
        ...
        if (chooser.showSaveDialog(null) == JFileChooser.APPROVE_OPTION) {
            ...
            JOptionPane.showMessageDialog(ventanaTrabajador, "No hay datos para exportar","Mensaje de error",JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == this.panelInventario.btnAgregar) {
            Emergente_Panel_Almacen panel = new Emergente_Panel_Almacen(ventanaTrabajador, true);
            ...
            if (listaAgregar == null) {// caso que presiono cancelar
                ...
                Linked_List.InventarioTEMP busqueda = lista_vehiculos.findIndexId(lista_vehiculos, id);

                if (busqueda.isFunciona()) {
                    Emergente_Aviso mensaje = new Emergente_Aviso(ventanaTrabajador, true, "El ID del artículo ya existe, utilize otro ID o actualize el artículo ");
                    ...
                    if (consultas.create(temp_model)) {
                        ...
                        Emergente_Aviso mensaje = new Emergente_Aviso(ventanaTrabajador, true, "No se pudo agregar al inventario");
                        mensaje.setVisible(true);
                    }
                }
        ...
        if (e.getSource()
            ...
            if (this.panelInventario.jTable1.getSelectionModel().isSelectionEmpty() == false) {
                ...
                if (consultas.delete(temp_model)) {
                    ...
                    this.llenarTablaAlmacen(lista_vehiculos);
                    Emergente_Aviso mensaje = new Emergente_Aviso(ventanaTrabajador, true, "Se elimino del inventario");
                    mensaje.setVisible(true);
                } else {
                    Emergente_Aviso mensaje = new Emergente_Aviso(ventanaTrabajador, true, "No se logro eliminar");
                    mensaje.setVisible(true);
                }

            } else {
                Emergente_Aviso mensaje = new Emergente_Aviso(ventanaTrabajador, true, "Seleccione una fila a eliminar");
                mensaje.setVisible(true);
            ...
            if (this.panelInventario.jTable1.getSelectionModel().isSelectionEmpty() == false) {
                if (this.panelInventario.jTable1.isEditing()) {
                    ...
                    if (consultas.update(temp_model)) {
                        ...
                        this.llenarTablaAlmacen(lista_vehiculos);
                        Emergente_Aviso mensaje = new Emergente_Aviso(ventanaTrabajador, true, "Se actualizo el registro");
                        mensaje.setVisible(true);
                    } else {
                        Emergente_Aviso mensaje = new Emergente_Aviso(ventanaTrabajador, true, "No se pudo actualizar el registro");
                        mensaje.setVisible(true);
                    }
                }
            } else {
                Emergente_Aviso mensaje = new Emergente_Aviso(ventanaTrabajador, true, "Seleccione una fila a modificar");
                mensaje.setVisible(true);
    ...
    private void mensaje(String msg) {
        Emergente_Aviso mensajes = new Emergente_Aviso(ventanaTrabajador, true, msg);
