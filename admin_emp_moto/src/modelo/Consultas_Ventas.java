...
public class Consultas_Ventas extends ConexionSQL {
    ...
    public int create(Modelo_Ventas ventas) {
        ...
        String query = "INSERT INTO Ventas "
                + "( id, id_factura, id_prod,id_trabajador,id_cliente,fecha_hora,monto_inicial,cuotas,dscto)"
                + " values ( ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        try {
            ps = con.prepareStatement(query);
            ps.setInt(1, ventas.getId());
            ps.setInt(2, ventas.getId_factura());
            ps.setInt(3, ventas.getId_prod());
    ...
    public boolean read(Modelo_Ventas ventas) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                ventas.setId_factura(rs.getInt("id_factura")); 
                ventas.setId_prod(rs.getInt("id_prod"));
    ...
    public boolean update(Modelo_Ventas ventas) {
        ...
        String query = "UPDATE Ventas SET "
                ...
                + " monto_inicial = ?, cuotas =?, dscto=? "
                + "WHERE id = ? ";

        try {
            ps = con.prepareStatement(query);
            
            ps.setInt(1, ventas.getId_factura());
            ps.setInt(3, ventas.getId_prod());
            ps.setInt(2, ventas.getId_trabajador());
